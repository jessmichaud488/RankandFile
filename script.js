...
function displayPostArticles(postArray) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        buildPostOutput += `<h4><a href="${postArray.articles[i].url}" target="_blank">${postArray.articles[i].title}</a></h4>`;
        buildPostOutput += `<img src="${postArray.articles[i].urlToImage}" alt="Article Image">`;
...
function getPoliticianId() {
    $("ul#senate, ul#house").on('click', 'li a', function (event) {
        event.preventDefault();
